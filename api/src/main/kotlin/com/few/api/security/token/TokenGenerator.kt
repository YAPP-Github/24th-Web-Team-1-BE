...
class TokenGenerator(
    ...
    fun generateAuthToken(memberId: Long?, memberRoles: List<Roles>): AuthToken {
        return AuthToken(generateAccessToken(memberId, memberRoles, accessTokenValidTime), generateRefreshToken(memberId, memberRoles, refreshTokenValidTime))
    }

    fun generateAuthToken(memberId: Long?, memberRoles: List<Roles>, accessTokenValidTime: Long?, refreshTokenValidTime: Long?): AuthToken {
        return AuthToken(generateAccessToken(memberId, memberRoles, accessTokenValidTime), generateRefreshToken(memberId, memberRoles, refreshTokenValidTime))
    }

    fun generateAccessToken(memberId: Long?, memberRoles: List<Roles>, accessTokenValidTime: Long?): String {
        val now = Date()
    ...
    fun generateRefreshToken(memberId: Long?, memberRoles: List<Roles>, refreshTokenValidTime: Long?): String {
        val now = Date()
        val roles = convertToStringList(memberRoles)
