    ...
    BehaviorSpec({
        ...
        given("멤버의 워크북 구독 요청이 온 상황에서") {
            ...
            `when`("멤버의 구독 히스토리가 없는 경우") {
                every { subscriptionDao.selectSubscriptionSendStatus(any()) } returns
                    listOf(
            ...
            `when`("이미 구독한 히스토리가 있고 구독이 취소된 경우") {
                ...
                val day = 2
                every { subscriptionDao.selectTopWorkbookSubscriptionStatus(any()) } returns
            ...
            `when`("구독 중인 경우") {
                ...
                val day = 2
                every { subscriptionDao.selectTopWorkbookSubscriptionStatus(any()) } returns
                    WorkbookSubscriptionStatus(workbookId = workbookId, isActiveSub = true, day)
