...
class BrowseUndoneProblemsUseCaseTest :
    BehaviorSpec({
        ...
        given("밀린 문제 ID 조회 요청이 온 상황에서") {
            ...
            `when`("밀린 문제가 존재할 경우") {
                every { problemSubscriptionService.browseWorkbookIdAndProgress(any()) } returns
                    listOf(
                ...
                every { problemDao.selectProblemIdByArticleIds(any()) } returns
                    listOf(
                        ProblemIdAndArticleIdRecord(1L, 2L),
                        ProblemIdAndArticleIdRecord(2L, 2L),
                    )

                every { submitHistoryDao.selectProblemIdByProblemIds(any()) } returns
                    SubmittedProblemIdsRecord(
                        listOf(
                            1L,
                            2L,
                        ),
