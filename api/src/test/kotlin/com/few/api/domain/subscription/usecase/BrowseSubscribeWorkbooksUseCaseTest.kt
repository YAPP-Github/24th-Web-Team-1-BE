...
class BrowseSubscribeWorkbooksUseCaseTest : BehaviorSpec({
    ...
    beforeContainer {
        ...
        useCase = BrowseSubscribeWorkbooksUseCase(
            subscriptionDao,
    ...
    given("메인 카드 뷰에서 멤버의 구독 워크북 정보를 조회하는 경우") {
        ...
        `when`("멤버의 구독 워크북 정보가 존재할 경우") {
            ...
            every { subscriptionDao.selectAllSubscriptionSendStatus(any()) } returns listOf(
            ...
            every { objectMapper.writeValueAsString(any()) } returns "{\"articleId\":$activeWorkbookId}" andThen "{\"articleId\":$inactiveWorkbookId}"
    ...
    given("마이 페이지에서 멤버의 구독 워크북 정보를 조회하는 경우") {
        ...
        `when`("멤버의 구독 활성 워크북 정보이 존재할 경우") {
            ...
            every { subscriptionWorkbookService.readAllWorkbookTitle(any()) } returns mapOf(
