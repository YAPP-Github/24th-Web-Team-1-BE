    ...
    BehaviorSpec({
        ...
        given("특정 문제를 조회하는 요청이 온 상황에서") {
            ...
            `when`("문제가 존재할 경우") {
                ...
                val problemContents = "{}"
                val articleId = 3L
                every { problemDao.selectProblemContents(any()) } returns
                    SelectProblemRecord(id = problemId, title = title, contents = problemContents, articleId = articleId)

                val contentCount = 2
                every { contentsJsonMapper.toObject(any()) } returns
                    Contents(
                        IntStream
                            .range(1, 1 + contentCount)
                            .mapToObj { Content(number = it.toLong(), content = "{}") }
                            .toList(),
                    )
