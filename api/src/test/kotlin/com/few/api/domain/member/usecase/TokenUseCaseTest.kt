    ...
    BehaviorSpec({
        ...
        given("리프레시 토큰이 포함된 토큰 갱신 요청이 온 상황에서") {
            ...
            `when`("유효한 리프레시 토큰인 경우") {
                ...
                every { tokenGenerator.generateAuthToken(any(), any(), any()) } returns
                    AuthToken(
        ...
        given("멤버 아이디 정보를 암호화한 토큰이 포함된 토큰 갱신 요청이 온 상황에서") {
            ...
            `when`("멤버 인증 토큰이 유효하고 인증을 위한 요청인 경우") {
                ...
                val accessToken = "newAccessToken"
                val refreshToken = "newRefreshToken"
                every { tokenGenerator.generateAuthToken(any(), any(), any(), any(), any()) } returns
                    AuthToken(
                        accessToken = accessToken,
                        refreshToken = refreshToken,
                    )
                ...
                val email = "test@gmail.com"
                every { memberDao.selectMemberEmailAndType(any()) } returns
